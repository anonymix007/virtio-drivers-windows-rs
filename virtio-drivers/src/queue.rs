...
use core::mem::{MaybeUninit, size_of, take};
#[cfg(test)]
use core::ptr;
...
impl<H: Hal, const SIZE: usize> VirtQueue<H, SIZE> {
    ...
    /// Returns a new in-place VirtQueue initializer.
    ///
    ...
    pub fn init<T: Transport>(
        ...
        init_scope(move || {
            ...
            let init_desc_shadow = init_array_from_fn(|i| Descriptor {
                ...
                next: if i < SIZE { (i + 1) as u16 } else { 0 },
            });
    ...
    pub fn new<T: Transport>(
        ...
        let mut q = MaybeUninit::<Self>::uninit();
        let initializer = Self::init(transport, idx, indirect, event_idx, access_platform);
        Ok(unsafe {
            // SAFETY: `q` is not initialized and can be overwritten, pointer to it
            // is valid and unique
            initializer.__init(q.as_mut_ptr())?;
            // SAFETY: `q` was successfully initialized
            q.assume_init()
        })
    }
